...
export function EmisionDirectaAction({ empresaTipo }: { empresaTipo?: string | null }) {
  ...
  return (
    <>
      <style>{`
        .ed-overlay{position:fixed;inset:0;z-index:80;display:flex;align-items:center;justify-content:center;padding:24px;background:rgba(0,0,0,.58);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);animation:edFadeIn .2s ease both}
        .ed-panel{width:min(760px,96vw);height:min(760px,88vh);border-radius:20px;overflow:hidden;background:var(--surface);border:1px solid var(--border);box-shadow:0 30px 90px rgba(0,0,0,.45),inset 0 1px 0 var(--border);display:flex;flex-direction:column}
        ...
        .sparkle-icon{width:28px;height:28px;border-radius:7px;display:flex;align-items:center;justify-content:center;background:rgba(232,85,62,.12);color:currentColor;flex-shrink:0;transition:background var(--transition),color var(--transition)}
        .sparkle-button:is(:hover,:focus-visible) .sparkle-icon{background:hsl(var(--accent-h) var(--accent-s) var(--accent-l) / .22);color:white}
  ...
  return (
    <>
      <style>{`
        ...
        .mass-sparkle-button{--active:0;--transition:.3s;--spark:1.8s;--cut:0px;--accent-h:77;--accent-s:88%;--accent-l:55%;--bg:radial-gradient(40% 50% at center 100%,hsl(var(--accent-h) calc(var(--active) * 88%) 70% / var(--active)),transparent),radial-gradient(80% 100% at center 120%,hsl(var(--accent-h) calc(var(--active) * 88%) 56% / var(--active)),transparent),hsl(var(--accent-h) calc(var(--active) * 88%) calc((var(--active) * 28%) + 16%));position:relative;display:flex;align-items:center;gap:10px;width:100%;padding:10px 14px;border:0;border-bottom:1px solid rgba(180,240,39,.10);border-radius:0;background:rgba(180,240,39,.06);color:#b4f027;cursor:pointer;text-align:left;white-space:nowrap;box-shadow:0 0 calc(var(--active) * 2em) calc(var(--active) * .35em) hsl(var(--accent-h) var(--accent-s) var(--accent-l) / .24),inset 0 0 0 1px rgba(180,240,39,.04),0 -.05em 0 0 hsl(var(--accent-h) calc(var(--active) * 88%) calc(var(--active) * 58%));transition:box-shadow var(--transition),background var(--transition),color var(--transition);overflow:hidden}
        .mass-sparkle-button:is(:hover,:focus-visible){--active:1;background:var(--bg);color:#2f5a0d;outline:none}
        ...
        .mass-icon{width:28px;height:28px;border-radius:7px;border:1px solid rgba(180,240,39,.14);display:flex;align-items:center;justify-content:center;background:rgba(180,240,39,.08);color:currentColor;flex-shrink:0;box-shadow:0 0 18px rgba(180,240,39,.06);transition:background var(--transition),color var(--transition),border-color var(--transition)}
        .mass-sparkle-button:is(:hover,:focus-visible) .mass-icon{background:hsl(var(--accent-h) var(--accent-s) var(--accent-l) / .34);border-color:hsl(var(--accent-h) var(--accent-s) var(--accent-l) / .5);color:#2f5a0d}
        ...
        .mass-sparkle-button:is(:hover,:focus-visible) .mass-subtitle{color:rgba(47,90,13,.72)}
        ...
        .mass-receipts .mass-paper{fill:rgb(180 240 39 / .03);stroke:currentColor;stroke-width:1.45;stroke-linejoin:round;filter:drop-shadow(0 0 calc(var(--active) * 8px) hsl(var(--accent-h) var(--accent-s) var(--accent-l) / .58));transition:stroke var(--transition),filter var(--transition),fill var(--transition)}
        ...
        <div className="md-overlay" onClick={() => setOpen(false)}>
          <div className="md-panel" onClick={(e) => e.stopPropagation()} style={{ maxHeight: "90vh" }}>
            ...
            <div style={{ padding: "12px 20px", borderTop: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0, background: "var(--surface)" }}>
              ...
              <a href="/api/generar-template" style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 10, border: "1px solid rgba(180,240,39,.18)", background: "rgba(180,240,39,.06)", color: "#b4f027", fontSize: 10, fontWeight: 700, textDecoration: "none", cursor: "pointer" }}>
