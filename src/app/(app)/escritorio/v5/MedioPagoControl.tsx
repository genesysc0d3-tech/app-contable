    ...
    <div style={{ display: "flex", alignItems: "center", gap: 9, marginTop: 0, minWidth: 0 }}>
      ...
      {open && pos && typeof document !== "undefined" && createPortal(
        <div
          ...
          className="z-[200] w-64 rounded-xl bg-white dark:bg-[#1c1c1e] border border-[var(--border)] shadow-[0_12px_32px_rgba(0,0,0,0.18)] dark:shadow-[0_12px_32px_rgba(0,0,0,0.5)] overflow-y-auto animate-fade-in py-2"
        >
          ...
          <button
            ...
            aria-selected={!medio}
            onClick={() => cambiar("")}
            className={`w-[calc(100%-12px)] mx-1.5 rounded-lg flex items-center gap-2.5 px-3 py-2.5 text-left transition-colors ${!medio ? "bg-[color-mix(in_srgb,var(--foreground)_6%,transparent)]" : "hover:bg-[color-mix(in_srgb,var(--foreground)_4%,transparent)]"}`}
          >
            <div className="w-3 h-3 shrink-0 text-[#E8553E]">{!medio && <Check size={12} weight="bold" />}</div>
            <div className="flex-1 min-w-0">
              <p className="text-[11.5px] font-semibold leading-snug">Sin elegir</p>
              {esCartola && <p className="text-[10px] text-[var(--muted-light)] mt-1 leading-snug">Las boletas saldrían como Efectivo</p>}
            </div>
          </button>
          {MEDIOS_PAGO_SII.map((m) => (
            <button
              ...
              onClick={() => cambiar(m)}
              className={`w-[calc(100%-12px)] mx-1.5 rounded-lg flex items-center gap-2.5 px-3 py-2.5 text-left transition-colors ${m === medio ? "bg-[color-mix(in_srgb,var(--foreground)_6%,transparent)]" : "hover:bg-[color-mix(in_srgb,var(--foreground)_4%,transparent)]"}`}
            >
              <div className="w-3 h-3 shrink-0 text-[#E8553E]">{m === medio && <Check size={12} weight="bold" />}</div>
              <div className="flex-1 min-w-0 flex items-center gap-2">
                <p className="text-[11.5px] font-semibold leading-snug">{m}</p>
                {m === sugerido && !medio && (
