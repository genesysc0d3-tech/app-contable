 ...
 *  - Extensión: icono massDTE → conectada con bóveda lista (criterio de Emitir).
  ...
  return (
    <span className="conector-chip" aria-label={title} style={{ display: "inline-flex", alignItems: "center", cursor: "default" }}>
      {/* La opacidad va SOLO en el logo: si envuelve al chip entero arrastra al
          tooltip (saldría translúcido) y en Safari el padre semi-transparente +
          transform del tooltip re-rasteriza la capa en cada hover — el icono
          "bailaba" (bug fundador 2026-09-02). */}
      <span style={{ display: "inline-flex", alignItems: "center", gap: 4, opacity: ok ? 1 : 0.55 }}>
        {children}
      </span>
      <Badge ok={ok} />
...
function LogoMcp({ size = 19 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="10 10 172 172" fill="none" stroke="currentColor" strokeWidth="12" strokeLinecap="round" style={{ display: "block" }} aria-hidden>
...
// Logos OFICIALES (paths de Simple Icons — la gente los reconoce al tiro).
function LogoClaude() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="#D97757" style={{ display: "block" }} aria-hidden>
...
function LogoTelegram({ size = 19 }: { size?: number }) {
  return (
