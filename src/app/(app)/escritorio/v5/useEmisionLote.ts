...
export function useEmisionLote(args: { empresaId: string; empresaRut?: string | null }) {
  ...
  const closeJob = useCallback(async (jobId: string, estado: "failed" | "cancelled") => {
    try {
  ...
  const iniciar = useCallback(async (items: ItemLoteEmision[]) => {
    ...
    const driver = {
      ...
      async emitirUna(item: ItemLote, reportar: (s: string) => void): Promise<DesenlaceItem> {
        ...
        // 5. soltar el lock si NO emitió (en emitida el server ya lo soltó en /result)
        if (desenlace.estado !== "emitida") await closeJob(job.jobId, desenlace.estado === "fallida" ? "failed" : "cancelled");
        return desenlace;
      },
    };
  ...
  return { progreso, pausa, corriendo, iniciar, detener, responderPausa };
}
