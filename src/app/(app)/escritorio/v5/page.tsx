...
export default async function V5Page({ searchParams }: {
  ...
  const [rcvData, propsData, clData, calProps, calDocs, pendCountData, aprobCountData] = await Promise.all([
...
body{font-family:'DM Sans',sans-serif;background:#0f1014;color:#e8eaf0}
:root{--accent:#E8553E;--accent-light:rgba(232,85,62,.1);--surface:rgba(255,255,255,.04);--border:rgba(255,255,255,.06);--muted:#636878;--muted-light:#888}
.app{display:grid;grid-template-columns:3fr 7fr;max-width:1400px;margin:0 auto;gap:24px;height:calc(100vh - 40px);padding:0 0}
.panel-hd-txt .plantilla{margin-left:auto;display:flex;align-items:center;gap:3px;padding:4px 8px;border-radius:5px;border:1px solid rgba(255,255,255,.06);background:transparent;color:#636878;font-size:9px;font-weight:500;cursor:pointer;white-space:nowrap;transition:all .15s}
.panel-hd-txt .plantilla:hover{background:rgba(255,255,255,.04);color:#e8eaf0}
...
.dz-txt p{font-size:10px;color:#636878;margin-top:1px}
...
.doc-card{background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.04);border-radius:10px;overflow:hidden;transition:all .15s}
...
.dh .mt{font-size:9px;color:#636878;flex-shrink:0}
...
.om-it{display:flex;align-items:center;gap:6px;padding:4px 6px;font-size:9px;color:#636878;border-radius:4px}
...
.prh{display:flex;justify-content:space-between;font-size:9px;color:#636878}
...
.prl{font-size:8px;color:#636878;display:flex;align-items:center;gap:3px}
...
.hist-btn{width:100%;display:flex;align-items:center;justify-content:space-between;padding:6px 0;border:none;background:none;cursor:pointer;font-size:10px;font-weight:600;color:#e8eaf0;border-top:1px solid rgba(255,255,255,.04);transition:all .15s}
...
.cal{padding:12px 16px;border-bottom:1px solid rgba(255,255,255,.04);flex-shrink:0}
.cal-h{display:flex;align-items:center;gap:6px;margin-bottom:8px}
.cal-h .m{font-size:11px;font-weight:600;color:#e8eaf0}
.cal-h .nv{font-size:9px;color:#636878;cursor:pointer;padding:2px 4px;border-radius:3px;text-decoration:none}
.cal-h .nv:hover{color:#e8eaf0;background:rgba(255,255,255,.04)}
.cal-h .cl{font-size:9px;color:#636878;cursor:pointer;margin-left:auto;padding:2px 6px;border-radius:3px;text-decoration:none}
...
.cal-day{width:26px;padding:3px 0;display:flex;flex-direction:column;align-items:center;border-radius:4px;cursor:pointer;text-decoration:none;color:#636878;transition:all .15s}
...
.topbar{display:flex;align-items:center;justify-content:space-between;padding:8px 16px;border-bottom:1px solid rgba(255,255,255,.04);flex-shrink:0}
...
.topbar-r .stat .lbl{font-size:10px;color:#636878}
.topbar-r .sep{color:#2a2d36;font-size:9px}
.topbar-r .date{font-size:10px;color:#636878}
.tab-bar{display:flex;gap:3px;padding:10px 16px;border-bottom:1px solid rgba(255,255,255,.04);flex-shrink:0}
...
.dtabs{display:flex;gap:4px;padding:8px 16px;border-bottom:1px solid rgba(255,255,255,.04);overflow-x:auto;scrollbar-width:none;flex-shrink:0}
.dtabs::-webkit-scrollbar{display:none}
.dtab{padding:4px 10px;border-radius:6px;border:1px solid rgba(255,255,255,.04);cursor:pointer;font-size:9px;white-space:nowrap;background:transparent;color:#636878;display:flex;align-items:center;gap:4px;transition:all .15s;flex-shrink:0}
.dtab:hover{border-color:rgba(255,255,255,.1);color:#e8eaf0}
...
.em-header{display:flex;align-items:center;gap:10px;padding-bottom:10px;border-bottom:1px solid rgba(255,255,255,.04)}
.em-header .big{font-size:20px;font-weight:300}
.em-header .lbl{font-size:10px;color:#636878}
...
.em-header .rf{width:24px;height:24px;border-radius:6px;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,.04);color:#636878;font-size:12px;margin-left:auto}
.em-header .rf:hover{background:rgba(255,255,255,.08);color:#e8eaf0}
...
.em-pills .pl.ina{background:transparent;color:#636878}
.em-pills .pl.ina:hover{background:rgba(255,255,255,.04);color:#e8eaf0}
.em-pills .sc{font-size:9px;color:#636878;display:flex;align-items:center;gap:4px;margin-left:auto;padding:3px 6px;border-radius:4px;cursor:pointer}
.em-pills .sc:hover{background:rgba(255,255,255,.04)}
.em-item{display:flex;align-items:center;gap:8px;padding:8px 10px;border-radius:8px;border:1px solid rgba(255,255,255,.04);margin-bottom:4px;transition:all .15s}
...
.em-item .inf .tt{font-size:10px;font-weight:500;color:#e8eaf0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.em-item .inf .sub{font-size:9px;color:#636878;margin-top:1px}
...
.em-item .tp .ina{background:rgba(255,255,255,.04);color:#4a4d55}
.em-item .mo{font-size:11px;font-weight:600;text-align:right;min-width:56px;font-variant-numeric:tabular-nums}
.em-bar{position:sticky;bottom:0;padding:10px 16px;background:#16181d;border-top:1px solid rgba(255,255,255,.04);display:flex;align-items:center;justify-content:space-between;gap:10px;z-index:2}
.em-bar .l{font-size:10px;color:#636878}
.em-bar .l .b{font-weight:600;color:#e8eaf0}
...
.em-empty .ic{width:40px;height:40px;border-radius:10px;background:rgba(255,255,255,.04);display:flex;align-items:center;justify-content:center;margin:0 auto 8px;color:#636878}
.em-empty .ic svg{width:18px;height:18px}
.em-empty h4{font-size:12px;font-weight:500;margin-bottom:4px}
.em-empty p{font-size:10px;color:#636878;max-width:280px;margin:0 auto;line-height:1.4}
...
`}</style>
      ...
      <div style={{ fontFamily: "'DM Sans','Inter',sans-serif", color: "#e8eaf0", background: "#0f1014", minHeight: "100vh", padding:20 }}>

        {/* FLOATING CONTROLS */}
        <div className="floating" style={{position:"fixed",top:16,right:16,zIndex:50,display:"flex",alignItems:"center",gap:8}}>
          <Link href="/empresa" style={{textDecoration:"none"}}>
            <div className="btn" style={{width:36,height:36,borderRadius:10,background:"#16181d",border:"1px solid rgba(255,255,255,.06)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"#636878"}}>
          ...
          <div className="btn" style={{width:36,height:36,borderRadius:10,background:"#16181d",border:"1px solid rgba(255,255,255,.06)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"#636878",position:"relative"}}>
            ...
            <div style={{position:"absolute",top:6,right:6,width:7,height:7,borderRadius:"50%",background:"#b4f027",border:"1.5px solid #16181d"}} />
          </div>
        ...
        <div className="app">
          ...
          {/* ═══ LEFT COLUMN ═══ */}
          <div className="left-col" style={{display:"flex",flexDirection:"column",gap:16,overflowY:"auto",scrollbarWidth:"none"}}>

            {/* RCV CARD */}
            <div className="rcv-card" style={{background:"#16181d",borderRadius:20,padding:"14px 18px",border:"1px solid rgba(255,255,255,.04)"}}>
              <div className="rcv-h" style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
                ...
                <span style={{fontSize:11,fontWeight:600,color:"#e8eaf0"}}>Registro de Ventas</span>
                <span className="mes" style={{fontSize:9,color:"#636878",fontWeight:500}}>{mes}</span>
              </div>
              <div className="rcv-sub" style={{fontSize:10,color:"#636878",marginBottom:6}}>
                <strong style={{fontWeight:600,color:"#e8eaf0"}}>{rcvTotal.docs}</strong> boletas emitidas este mes
              </div>
              <div className="rcv-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:5}}>
                {[{l:"Neto",v:fmt(rcvTotal.neto)},{l:"IVA",v:fmt(rcvTotal.iva)},{l:"Exento",v:fmt(rcvTotal.exento)},{l:"Total",v:fmt(rcvTotal.total),tot:true}].map((x,i) => (
                  <div key={i} className="item" style={{padding:6,borderRadius:6,background:"rgba(255,255,255,.03)",textAlign:"center"}}>
                    <div className="lbl" style={{fontSize:8,color:"#636878",marginBottom:1}}>{x.l}</div>
                    <div className={`val${x.tot?" tot":""}`} style={{fontSize:12,fontWeight:x.tot?700:600,color:x.tot?"#b4f027":"#e8eaf0"}}>{x.v}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* EMITIR PANEL */}
            <div className="panel" style={{flex:1,minHeight:0,background:"#16181d",borderRadius:20,border:"1px solid rgba(255,255,255,.04)",display:"flex",flexDirection:"column",overflow:"hidden"}}>
              <div className="panel-hd" style={{display:"flex",alignItems:"center",gap:12,padding:"14px 18px",borderBottom:"1px solid rgba(255,255,255,.04)",flexShrink:0}}>
                ...
                <div className="panel-hd-txt" style={{display:"flex",alignItems:"center",gap:8,flex:1}}>
                  <div><h2 style={{fontSize:13,fontWeight:600,color:"#e8eaf0"}}>Emitir</h2><p style={{fontSize:10,color:"#636878",marginTop:1}}>Subí cartola o Excel modelo</p></div>
                  <Link href="/api/generar-template" className="plantilla" style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:3,padding:"4px 8px",borderRadius:5,border:"1px solid rgba(255,255,255,.06)",background:"transparent",color:"#636878",fontSize:9,fontWeight:500,textDecoration:"none"}}>
          ...
          {/* ═══ RIGHT COLUMN ═══ */}
          <div className="right-col" style={{display:"flex",flexDirection:"column",background:"#16181d",borderRadius:20,border:"1px solid rgba(255,255,255,.04)",boxShadow:"0 4px 16px rgba(0,0,0,.2)",overflow:"hidden"}}>
                ...
                <div className="r-scroll">
                  <div className="sec">
                    <div className="bl-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"0 0 8px"}}>
                      <span style={{fontSize:10,color:"#e8eaf0",fontWeight:600}}>Últimas emitidas</span>
                      <Link href="/boletas/reportes" style={{fontSize:9,color:"#636878",textDecoration:"none",display:"flex",alignItems:"center",gap:4,fontWeight:600}}>
                    ...
                    {(boletas ?? []).length === 0 ? (
                      <div style={{textAlign:"center",padding:"24px 16px"}}>
                        <p style={{fontSize:10,color:"#636878"}}>No hay boletas emitidas aún</p>
                      </div>
                    ) : (
                      boletas!.map(b => {
                        ...
                        return (
                          <div key={b.id} className={`bl-item ${esAnulada ? "an" : ""}`}
                            ...
                            <div className="ic" style={{width:28,height:28,borderRadius:6,background:"rgba(255,255,255,.04)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,color:"#636878"}}>
                            ...
                            <div className="inf" style={{flex:1,minWidth:0}}>
                              <div className="top" style={{display:"flex",alignItems:"center",gap:4,fontSize:10,fontWeight:600,color:"#e8eaf0"}}>
                                <span className="fl" style={{color:"#e8eaf0"}}>#{b.folio}</span>
                                <span className={`bd ${b.tipo_dte === 39 ? "af" : b.tipo_dte === 41 ? "ex" : "an"}`}
                                  style={{fontSize:7,padding:"1px 5px",borderRadius:8,fontWeight:600,
                                    background: b.tipo_dte === 39 ? "var(--accent-light)" : b.tipo_dte === 41 ? "rgba(59,130,246,.1)" : "rgba(255,255,255,.04)",
                                    color: b.tipo_dte === 39 ? "#E8553E" : b.tipo_dte === 41 ? "#5b9cf6" : "#636878",
                                  }}
                                >{b.tipo_dte === 39 ? "AFECTA" : b.tipo_dte === 41 ? "EXENTA" : `DTE ${b.tipo_dte}`}</span>
                                {esAnulada && (
                                  <span className="bd an" style={{fontSize:7,padding:"1px 5px",borderRadius:8,fontWeight:600,background:"rgba(255,255,255,.04)",color:"#636878"}}>ANULADA</span>
                                )}
                              </div>
                              <div className="sub" style={{fontSize:9,color:"#636878",marginTop:1}}>
                                {b.receptor_razon_social ?? "Sin receptor"} · {new Date(b.fecha_emision).toLocaleDateString("es-CL",{day:"numeric",month:"short",year:"numeric"})}
                              </div>
                            </div>
