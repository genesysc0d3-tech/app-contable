...
describe("la disciplina de fases sigue en pie", () => {
  ...
  it("el borrado es el último control de la página", () => {
    ...
    for (const control of ["<PlanToggle", "<TrialCortesiaToggle", "<ReiniciarTrialButton", "<CarrilEmisionSelector", "<MigrarEmpresaForm"]) {
      expect(detalle.indexOf(control)).toBeLessThan(purgar);
    }
  });

  it("cada control que escribe tiene su Explica", () => {
    // Siete bloques Explica: ver como cliente, plan, prestar la prueba,
    // reiniciar la prueba, carril de emisión, traer empresa, borrar.
    const explicas = detalle.match(/<Explica/g) ?? [];
    expect(explicas.length).toBe(7);
  });
