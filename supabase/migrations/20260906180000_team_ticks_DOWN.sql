-- DOWN de 20260906180000_team_ticks.sql
-- Deshace la puerta del team: vuelve la regla RLS sin ticks, la RPC de
-- invitación con su firma vieja, borra la tabla de ticks y las RPC team_*.
-- Business vuelve a 1 persona incluida. Las filas que destruye: los ticks
-- (cuenta_usuario_empresas) y la columna empresas_permitidas de las
-- invitaciones pendientes — anotar antes de correr si hay alguna.

drop function if exists public.team_revocar_invitacion(uuid, uuid);
drop function if exists public.team_quitar_miembro(uuid, uuid, uuid);
drop function if exists public.team_actualizar_ticks(uuid, uuid, uuid[], uuid);
drop function if exists public.team_es_titular(uuid, uuid);
...
drop table if exists public.cuenta_usuario_empresas;

drop function if exists public.crear_empresa_invitacion_titular(uuid, text, text, text, uuid, timestamptz, uuid[]);
alter table public.empresa_invitaciones drop column if exists empresas_permitidas;

update public.planes_config
set personas_incluidas = 1,
    features = (
      select coalesce(jsonb_agg(
        case when f::text like '"Equipo%'
          then to_jsonb('Equipo: 1 persona incluida, las demás se suman aparte'::text)
          else f end), features)
      from jsonb_array_elements(features) as f),
    updated_at = now()
where codigo = 'business';

-- RPC de invitación con la firma anterior (copia literal de 20260620113000):
