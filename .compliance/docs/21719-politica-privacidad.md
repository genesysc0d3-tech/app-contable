# Política de Privacidad — AlphaCode SpA

**Última actualización:** 2026-06-25

## 1. Responsable del tratamiento
AlphaCode SpA, RUT 78.448.088-7, Av. Apoquindo 6410 Of. 605, Las Condes, Santiago. Contacto para datos personales: privacidad@massdte.cl.

## 2. Qué datos tratamos
Nombre, correo electrónico, RUT, y datos bancarios/tributarios que tú cargas o que se extraen de tus cartolas y comprobantes (movimientos, montos, nombres de contraparte, folios). Datos de uso de la aplicación. **No tratamos datos sensibles** (salud, biométricos) ni datos de niños, niñas y adolescentes de forma intencionada.

## 3. Finalidad y base de licitud
| Finalidad | Base de licitud |
|---|---|
| Prestación del servicio y gestión de tu cuenta | Ejecución de contrato |
| Procesar cartolas/comprobantes y proponer documentos tributarios | Ejecución de contrato |
| Emisión de documentos tributarios electrónicos (DTE) | Obligación legal (DL 825 / Código Tributario) |
| Cobro de la suscripción | Ejecución de contrato |
| Seguridad, prevención de fraude y soporte | Interés legítimo |

## 4. Con quién compartimos los datos
Usamos proveedores que actúan como encargados: **Supabase** (base de datos y almacenamiento), **Vercel** (hosting), **Cloudflare R2** (archivos), **OpenCode** (lectura por IA de cartolas/comprobantes), **Resend** e **ImprovMX** (correo), **Telegram** (recepción de comprobantes) y **Mercado Pago** (pagos). La base de datos de producción está en **Estados Unidos**. Existe además una **copia de respaldo inactiva en Brasil**, congelada al 22-08-2026, que se conserva solo como resguardo de continuidad y será destruida a más tardar el 31-12-2026. **No vendemos datos personales.**

Sobre el amparo de esas transferencias, lo decimos derechamente: **hoy se realizan bajo la Ley 19.628 y los contratos de servicio de cada proveedor. Ninguna cláusula contractual modelo está suscrita todavía.** Estamos incorporándolas con cada proveedor extranjero antes de que entre en vigencia la Ley 21.719 (1-dic-2026); mientras no estén firmadas, no afirmamos que la transferencia esté amparada en ellas.

**Qué ve la inteligencia artificial.** Antes de que el texto de tus movimientos salga de nuestro servidor, el nombre y el RUT de tus contrapartes se reemplazan por una etiqueta sin identidad (`PER_1`, `PER_2`); la correspondencia vive solo en memoria durante el procesamiento y se destruye al terminar. Dos excepciones dichas derechamente: (1) tu propio nombre y RUT sí viajan, porque sin ellos el sistema no sabe cuál de las dos partes de una transferencia eres tú; (2) cuando subes la **foto o el PDF** de un comprobante, la imagen va completa al servicio de lectura —hay que leerla entera para saber qué dice— y ahí pueden ir a la vista nombres y RUT impresos. El proveedor declara en su documentación que en los planes de pago no usa el contenido para entrenar modelos: es una declaración suya, **no un contrato firmado con nosotros**, y por eso mandamos lo mínimo y con la identidad tapada en vez de confiar solo en su palabra. Tampoco publica en qué regiones corre cada modelo. **La IA no decide nada sola: propone, y tú apruebas antes de emitir.**

**Qué ve tu asistente de IA, si decides conectarlo (opcional).** Si conectas tu propio asistente (Claude de Anthropic, ChatGPT de OpenAI u otro compatible) desde Empresa → Conector, ese asistente puede leer tus documentos pendientes y ordenarlos (dejarlos listos en Emitir o devolverlos a revisión); nunca emite, y lo cortas cuando quieras desde el mismo lugar. Le mandamos fecha, monto, tipo propuesto, estado y las razones de la clasificación, con la glosa del banco seudonimizada; de tus contrapartes, solo si tienen RUT y giro (sí/no) y el nombre como etiqueta sin identidad y acortado ("Juan P."). **No** le mandamos RUT, correo, teléfono, dirección, comuna ni giro de contrapartes, ni el nombre de tus archivos, ni claves. Lee un mes a la vez, de a 100, con tope diario por conector. Los datos van al proveedor del asistente que **tú** elegiste, bajo **sus** términos y tu cuenta con él — no bajo un contrato nuestro: el destinatario lo eliges tú (ver anexo de transferencias, §2.bis). El motivo que escribe tu asistente al mover un documento se guarda junto a ese documento como dato en cuarentena: nunca entrena un modelo ni se convierte en regla por sí solo. Conectar un asistente no cambia lo que pagas.

## 5. Por cuánto tiempo
Conservamos los datos mientras dure la relación contractual. Los documentos tributarios y la información contable se conservan **6 años** (Código Tributario). Luego se eliminan o anonimizan.

## 6. Tus derechos
Puedes ejercer **acceso, rectificación, supresión, oposición, portabilidad y bloqueo**, y **retirar tu consentimiento** cuando quieras, escribiendo a privacidad@massdte.cl o en el canal de derechos. Respondemos en **30 días corridos** (prorrogables una sola vez por 30 días más). La rectificación, supresión y oposición son siempre gratuitas; el acceso es gratuito al menos una vez por trimestre.

## 7. Decisiones automatizadas
Usamos tratamientos automatizados (OCR e IA) para **leer y clasificar** tus cartolas y comprobantes y **proponer** documentos. **Las propuestas no se ejecutan solas:** tú (o un usuario autorizado) revisas y apruebas antes de emitir. Tienes derecho a una explicación, a intervención humana y a oponerte. Si conectas un asistente de IA externo (§4), este puede dejar documentos listos o devolverlos a revisión, pero tampoco emite: la emisión sigue siendo tuya.

## 8. Origen de los datos
Provienen de ti directamente (registro, archivos que cargas, comprobantes por Telegram) y de los documentos que procesas en la plataforma.

## 9. Seguridad
Aplicamos medidas técnicas y organizativas: TLS/HSTS, cifrado en reposo, hashing de contraseñas, **aislamiento por cuenta (RLS) verificado con pruebas**, logs de auditoría, secretos fuera del código y minimización de datos. *(MFA en accesos de operador: en implementación.)*

## 10. Cambios
Podemos actualizar esta política; publicaremos la versión vigente con su fecha.

## 11. Reclamos
Puedes reclamar ante la **Agencia de Protección de Datos Personales** de Chile.

---
*Borrador generado con compliance-cl (pack ley-21719). No constituye asesoría legal; revisar con un abogado antes de publicar.*
